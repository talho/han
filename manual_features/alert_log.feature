Feature: Viewing the HAN alert log
  ...
  Scenario: Viewing acknowledged alerts with alert responses
    ...
    And delayed jobs are processed
    And "jane.smith@example.com" has acknowledged the HAN alert "Hello World"
  ...
  Scenario: Viewing audience of Alert and acknowledgement log from view(show)
    ...
    And I press "Send"
    And delayed jobs are processed
    And "jane.smith@example.com" has acknowledged the HAN alert "Hello World" with "" 0 minutes later
    And I follow the acknowledge HAN alert link
    And "daniel@example.com" has acknowledged the HAN alert "Hello World" with "if you cannot respond" 0 minutes later
    And delayed jobs are processed
    When I am on the HAN alert log
    Then I can see the alert summary for "Hello World"
    When I click "View" on "Hello World"
